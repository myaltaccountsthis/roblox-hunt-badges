...
export const HUB_BONUS = [{ badgeId: '244677413669115', badgeName: 'Dominus Infinitum' }, { badgeId: '2398335401043156', badgeName: 'Amictus Infinitum' }];
...
export async function request(url, asText = false, fetcher = fetch) {
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    const error = new Error(response.status === 429 ? "Rate limited by Roblox. Please wait before trying again." : `Roblox returned HTTP ${response.status}`);
    ...
    if (delay > 5000) throw new Error(response.status === 429 ? "Rate limited by Roblox. Please wait before trying again." : "Roblox is temporarily unavailable. Please try again shortly.");
...
export function gameEntry(game, badges, checkedAt) {
  // Explicit catalog selections win over newest-badge discovery. Live details still refresh by ID.
  if (game.pinned && game.badgeId) return { ...game, checkedAt, discoveryError: null };
...
export async function discover(existing = []) {
  ...
  const gameBadges = await mapLimit(games, 3, async game => {
    ...
    const pinned = existing.find(entry => entry.group === "game" && entry.universeId === game.universeId && entry.pinned && entry.badgeId);
    return gameEntry(pinned ? { ...game, ...pinned } : game, badges, checkedAt);
  });
  const hubBadges = [...HUB_BONUS, ...HUB_IDS.map((badgeId, i) => ({ badgeId, badgeName: `S${String(i + 1).padStart(2, '0')}` }))].map(({ badgeId, badgeName }) => ({ badgeId, badgeName: existing.find(entry => entry.group === 'hub' && entry.badgeId === badgeId)?.badgeName ?? badgeName, group: 'hub', status: badgeName.startsWith('S') ? 'confirmed' : 'provided', game: 'The Hunt: Roblox 20', universeId: HUB, rootPlaceId: '74205509034203', year: null, note: existing.find(entry => entry.group === 'hub' && entry.badgeId === badgeId)?.note ?? (badgeName.startsWith('S') ? 'User-provided hub badge.' : 'Hub item badge awarded for attaining ' + badgeName + '.'), checkedAt, badgeUrl: `https://www.roblox.com/badges/${badgeId}`, gameUrl: 'https://www.roblox.com/games/74205509034203' }));
